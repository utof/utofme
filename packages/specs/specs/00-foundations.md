...
- [ ] Self-hosted mono `.woff2` returns `Cache-Control: public, max-age=31536000, immutable` from Workers Assets — **verified in production** via `curl -I` against the deployed `*.workers.dev` URL after `deploy.yml` lands. The local Playwright harness runs against `astro preview`, which does **not** emit immutable cache headers (those come from Cloudflare Workers Assets at the edge); the corresponding test in `tests/e2e/typography.spec.ts` is therefore an explicit `test.skip` with this deferral as its skip reason.
