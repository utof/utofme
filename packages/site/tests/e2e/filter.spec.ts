// Why: e2e tests for FilterBar.svelte island on `/`.
...
test.describe("FilterBar — JS enabled", () => {
	...
	test("4. back button restores prior filter", async ({ page }) => {
		...
		await page.goto("/?type=video&tag=overview");
		await page.waitForSelector("[data-filter-bar]");
...
test.describe("FilterBar — JS disabled", () => {
	test("5. no-JS: full grid renders, filter bar hidden, no console error", async ({ browser }) => {
